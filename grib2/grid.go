...
func ReadGrid(f io.Reader, templateNumber uint16) (Grid, error) {
	...
	case 30:
		...
		grid.La1 = fixNegLatLon(grid.La1)
		grid.Lo1 = fixNegLatLon(grid.Lo1)
		Name="Lambert conformal Polar stereographic projection "
		g = &grid
...
type Grid0 struct {
	// Name 						string	   `json:"name"` //"Latitude/longitude (or equidistant cylindrical, or Plate Carree) "
	GridHeader
...
type Grid10 struct {
	// Name 						string `json:"name"` //name :=  "Mercator"
	GridHeader
	Ni                          uint32 `json:"ni"`
	Nj                          int32  `json:"nj"`
	La1                         int32  `json:"la1"`
...
type Grid20 struct {
	// Name 						string `json:"name"`//name =  "Polar stereographic projection";
	GridHeader
	Nx                          uint32 `json:"nx"`
	Ny                          uint32 `json:"ny"`
	La1                         int32  `json:"na1"`
...
// Grid30 Definition Template 3.30: 
type Grid30 struct {
	// Name 						string `json:"name"`//name =  "Lambert conformal Polar stereographic projection ";
	GridHeader
	Nx                          uint32 `json:"nx"`
	Ny                          uint32 `json:"ny"`
	La1                         int32  `json:"la1"`
...
type Grid40 struct {
	// Name 						string `json:"name"`//name =  "Gaussian latitude/longitude ";
	GridHeader
	Ni                          uint32 `json:"ni"`
	Nj                          uint32 `json:"nj"`
	BasicAngle                  uint32 `json:"basicAngle"`
...
type Grid90 struct {
	// Name 						string `json:"name"`//name =  "Space view perspective or orthographic ";
	GridHeader
	Nx uint32 `json:"nx"`
	Ny uint32 `json:"ny"`
